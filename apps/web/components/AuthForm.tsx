...
export function AuthForm({ mode }: Props) {
  ...
  const submit = async (values: any) => { try { const result = mode === 'login' ? await login(values) : await register(values); localStorage.setItem('token', result.token); localStorage.setItem('role', result.user.role); router.push('/dashboard'); } catch (error) { setError('root', { message: error instanceof Error ? error.message : 'Request failed' }); } };
