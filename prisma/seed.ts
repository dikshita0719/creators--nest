...
async function main() {
  const admin = await prisma.user.upsert({
    ...
    update: {},
    create: { email: 'admin@example.com', role: UserRole.admin, profile: { create: { displayName: 'Marketplace Admin' } } },
  });
  const creator = await prisma.user.upsert({
    where: { email: 'creator@example.com' },
    update: {},
    create: { email: 'creator@example.com', role: UserRole.creator, profile: { create: { displayName: 'Alex Creator', bio: 'Documentary and event videographer', location: 'Portland' } }, creator: { create: { categories: ['events', 'documentary'], hourlyRate: 150 } } },
  });
  const client = await prisma.user.upsert({
    where: { email: 'client@example.com' },
    update: {},
    create: { email: 'client@example.com', role: UserRole.client, profile: { create: { displayName: 'Jamie Client' } } },
  });
  ...
  console.log(`Seeded ${admin.email}, ${creator.email}, ${client.email}, and sample booking.`);
}
